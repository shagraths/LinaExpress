...
class controllerSubirArchivo extends CI_Controller {
    ...
    function guardar_excelCliente() {
        ...
        foreach ($excel as $row) {
            if ($i==0) {
                    ...
                    $total++;
                    $ruta = './codigBarra/'.$fecha.'/';
                    $idCodigo = $this->modeloexcel->GuardarArchivoExcelCliente($i,$row[2],$row[3],$row[4],$row[5],$row[6],$row[7],$row[8],$row[9],$row[10],$id_persona,$nombre,$ruta);
                    if ( $idCodigo != 0) {
